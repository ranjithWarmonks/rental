...
class BulkStockAdjustmentView extends StatefulWidget {
  ...
  @override
  State<BulkStockAdjustmentView> createState() => _BulkStockAdjustmentViewState();
}

class _BulkStockAdjustmentViewState extends State<BulkStockAdjustmentView> {
  ...
  final List<StockAdjustmentRow> _rows = [
    StockAdjustmentRow(),
  ];
  ...
  Future<void> _saveAllAdjustments() async {
    ...
    try {
      await _stockAdjustmentService.submitBulkAdjustments(validRows);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Stock adjustments saved successfully!', style: TextStyle(fontFamily: 'Urbanist')),
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              ElevatedButton.icon(
                onPressed: _isSubmitting ? null : _saveAllAdjustments,
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // Main Adjustment Card Container
              Container(
                decoration: BoxDecoration(
                  color: Colors.white,
                ...
                child: Column(
                  ...
                  children: [
                    // Subtitle Note
                    Text(
                      'Fill in the adjustment rows below. Manual adjustments will immediately affect item stock levels.',
                      style: TextStyle(
                        fontFamily: 'Urbanist',
                        fontSize: 13,
                      ...
                      Container(
                        ...
                        child: const Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Expanded(flex: 3, child: Text('NOTES / REASON', style: TextStyle(fontFamily: 'Urbanist', fontWeight: FontWeight.bold, fontSize: 12, color: Colors.grey))),
                            SizedBox(width: 24),
                          ],
                        ),
                      ),
                    ...
                    _isLoadingItems
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (ctx, idx) {
                              final row = _rows[idx];
                              return _buildAdjustmentRow(row, idx);
                            },
                          ),
  ...
  Widget _buildAdjustmentRow(StockAdjustmentRow row, int index) {
    final isMobile = MediaQuery.of(context).size.width <= 600;
    if (isMobile) {
      return _buildMobileAdjustmentCard(row, index);
    }
    return _buildDesktopAdjustmentRow(row, index);
  }

  Widget _buildMobileAdjustmentCard(StockAdjustmentRow row, int index) {
    return Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      items: const [
                        DropdownMenuItem(
                          value: 'addition',
                          child: Text('Addition (+)', style: TextStyle(fontFamily: 'Urbanist', fontSize: 13, fontWeight: FontWeight.bold, color: Colors.blue)),
                        ),
                        DropdownMenuItem(
                          value: 'deduction',
                          child: Text('Deduction (-)', style: TextStyle(fontFamily: 'Urbanist', fontSize: 13, fontWeight: FontWeight.bold, color: Colors.red)),
                        ),
                        DropdownMenuItem(
                          value: 'set',
                          child: Text('Set Absolute Value', style: TextStyle(fontFamily: 'Urbanist', fontSize: 13, fontWeight: FontWeight.bold, color: Colors.purple)),
                        ),
                      ],
                      onChanged: (val) {
                        if (val != null) {
  ...
  Widget _buildDesktopAdjustmentRow(StockAdjustmentRow row, int index) {
    return Row(
        ...
        Expanded(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: Colors.white,
              borderRadius: BorderRadius.circular(10),
              border: Border.all(color: row.type == 'addition' ? Colors.blue.shade300 : (row.type == 'deduction' ? Colors.red.shade300 : Colors.purple.shade300)),
            ),
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                value: row.type,
